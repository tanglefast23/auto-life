...
**Round-5 rulings (dual external audits / P2.1, 2026-07-30):** anchors fire **at their target**, not at window-open — targetAt was dead data and the whole day ran an hour early (lunch 12:01, dinner 18:01, a 3h55m dead evening). With true targets restored, the authored 23:00 bedtime target proved untenable: the 16-hour waking day drains Energy through the urgent threshold nightly, contradicting §6.8's zero-urgent unattended week — **bedtime's v1 content target is therefore 22:30** (the engine semantics stay fire-at-target; the §7.1 sleep-continuation boundary stays the normative 23:00). Also ruled: Q4 consumption is **monotonic** (a prior-day block starting/deleted late never resurrects a consumed newer day); the stale-block sweep removes **AUTO members only**; `source` is immutable **provenance** and the player cap counts only *inserted* cards (promoting/moving an anchor or reactive pins it without consuming cap); **removing the running card stops it** (stop semantics, suppression included); practice **block status resets at wake**; recorded travel figures were undercounted one tick per journey (post-step sampling) — true figures 35/32 per day, corrected via the snapshot `processed` field with no behavior change.
...
- **Trace status [re-derived travel-inclusive at P2, ENGINE_VERSION 2]:** the recorded unattended week (seed 1234) reads, at the morning check from Day 2: **Energy 90 · Nutrition 77.4 · Movement 81.1 · Hygiene 95.4** (Day 2 Movement 75.0 — the ≥65 floor holds from Day 2 with no exemption). Movement sits *above* the pre-travel trace because **it-sticks fires daily unattended** (workout ends ~17:34, dinner *starts* ~18:05 — the §6.7 gap is end-to-start), halving Movement decay for 12 h — the P2 critics caught the first build measuring the gap at the meal's completion, which silenced the pair entirely. **Nutrition moved 82 → 77.4 (−4.6, beyond the ±3 tolerance)** — named honestly: travel shifts meal completions later, so the morning check catches less of breakfast's fill; the min-over-all-ticks (47.6, pre-breakfast) is the governing assertion and holds. **Transit measures 24–27 game-min/day** (band 20–50). Recovery from all-zeros: Health **77.3** at the Day-3 morning check (floor 75; the old "≥70 at end of Day 2" sampled the designed pre-breakfast trough — the round-1 sampling-error class). Neglect: Movement bottoms ≈17.8 within 3 days; the Stretch net floors it — felt consequence, never free-fall. **`content/harness-bands.json` is the normative source for every band**; §16.3 defers to it, and bands are never quietly widened to green a test.
...
| 22:30– | 22:30 [round 5] | Brush → Sleep | always |
...
- **Sleep window rules:** sleep targets 22:30 [round 5 — the original 23:00 target made the 16-hour day cross the urgent Energy threshold nightly, contradicting the zero-urgent week] and *ends at 07:00 regardless of start* — a late night is a short night, never a shifted calendar. If urgent sleep reaches Energy ≥80 between 23:00 and 07:00, the sim **continues sleeping until 07:00** (verified: this clause is the difference between next-morning recovery and a two-day wobble with a 13:30 afternoon nap).
...
- **Balance harness (CI), sampling-explicit (§6.5); `content/harness-bands.json` is normative for every numeric band (P2 ruling — the prose here describes, the data file governs):** the unattended golden seed deals **only benign wrinkles** — the §9.4 exposure guarantee is a scripted-player-week property, asserted there (verifier fix: rough-night and empty-fridge wrinkles legitimately break the fairness bands, by design). Untouched 7-day: every bar ≥65 at the **morning check (wake+2h; 09:00 baseline)** from Day 2; zero URGENT events; Nutrition min-over-all-ticks = 50±3 (pre-breakfast); dinner cap-waste ≤ 12; snack never fires except when a wrinkle engineers it (the §9.4 exposure guarantee is asserted separately: each reactive band visited ≥ once per scripted-player week). Neglect: no workouts → Movement <20 within 3 days. Recovery: all-bars-0 → Health ≥75 at the Day-3 morning check (re-derived at P2; the end-of-Day-2 sample hit the designed pre-breakfast trough). Property test: planner never moves a PINNED card across 10k random edit sequences. Note: Goal 6 requires player-pinned cards *on top of* what this autopilot assertion guarantees — the harness proves the world is fair; the goal still demands a hand on the wheel.
