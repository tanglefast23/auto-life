...
|---|---|---|
| **P0** | Safe scaffold, strict TS, package lock, test/content commands, CI workflow, current Skia web loading, SQLite WASM/headers, exported-build persistence proof | Clean-install verification; exported page visibly draws Skia; `crossOriginIsolated` is true; SQLite value survives hard reload and browser close/reopen on the same origin; CI actually runs green when a remote runner is available |
