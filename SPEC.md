...
- **Trace status [re-derived travel-inclusive at P2, per Q6 — no `ENGINE_VERSION` bump, no behavior change]:** the recorded unattended week (seed 1234) reads, at the morning check from Day 3 steady state: **Energy 90 · Nutrition 77.4 · Movement 69.1 · Hygiene 95.4** — all within ±3 of the pre-travel trace except Movement's Day-2 transient (**63**: the Day-1 start at 60 plus travel defers the first workout's full effect one day, so the ≥65 fairness floor applies **from Day 3**, with Day 2 holding a recorded ≥60 Movement floor). Nutrition min-over-all-ticks **47.5** pre-breakfast (band 50±3 holds at its low edge). **Transit measures ≈26 game-min/day** — below the original 30–50 estimate; the §6.8 transit line is re-derived to 20–50. Recovery from all-zeros reads Health ≈83 at the **Day-3 morning check** (the old "≥70 at end of Day 2" sampled the designed pre-breakfast trough — the same sampling error class round 1 fixed; re-derived to the morning-check sample). Neglect: with the workout anchor disabled, Movement bottoms at ≈17.8 within 3 days and the Stretch net floors it near 18 — felt consequence, never free-fall. Bands live in `content/harness-bands.json`; they are never quietly widened to green a test.
